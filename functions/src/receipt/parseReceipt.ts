...
function mergeMultiLineItems(items: ReceiptItem[]): ReceiptItem[] {
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    const isContinuation = 
      /^(\d+(\.\d+)?\s*(lt|ml|kg|g|cl|l|lb|pcs?|pkt|rech|a\d+|z\d+|s\d+|l\d+)\s*\(?[^)]*\)?)$/i.test(name) ||
      /^(\([^)]*\))$/.test(name) ||
 ...
 */
function hasOCRSpacingIssue(name: string): boolean {
  ...
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      const timeoutPromise = new Promise<never>((_, reject) =>
        setTimeout(() => reject(new Error('Le service met trop de temps à répondre')), 90000),
      ); // 90s timeout for video
...
async function parseWithGemini(
  ...
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      console.log(`🚀 [Gemini Parse] Using model: ${config.gemini.model}`);
      ...
      try {
        ...
        if (totalMatch || storeMatch) {
          console.log('📋 Found fallback data - total:', totalMatch?.[1], 'store:', storeMatch?.[1]);
          parsed = {
      ...
      if (deduplicatedItems.length === 0 && parsed.storeName && attempt === 0) {
        ...
        try {
          ...
          console.log('📦 FALLBACK parsed items count:', fallbackParsed.items?.length || 0);
          console.log('📦 FALLBACK raw text:', fallbackParsed.rawText?.substring(0, 500));
          ...
          if (fallbackParsed.items && fallbackParsed.items.length > 0) {
            ...
            const fallbackItems: ReceiptItem[] = fallbackParsed.items.map((item: any) => ({
              id: generateItemId(),
    ...
    try {
      ...
      await db.runTransaction(async transaction => {
        ...
        if (!scanCheck.canScan) {
          throw new functions.https.HttpsError(
            'resource-exhausted',
            scanCheck.reason || 'Limite de scans atteinte',
          );
        }
      ...
      console.log('📊 FINAL RESULT - Items:', parsedReceipt.items?.length, 'Total:', parsedReceipt.total);
      ...
      console.log('📊 [Validation] hasItems:', hasItems, '(count:', parsedReceipt.items?.length || 0, ')');
      ...
      if (!hasItems && !hasTotal && !hasStoreName) {
        ...
        throw new functions.https.HttpsError(
          'invalid-argument',
          'Impossible de lire les articles ou le total de ce reçu. Veuillez reprendre la photo avec un meilleur éclairage.',
        );
      } else {
      ...
      throw new functions.https.HttpsError(
        'internal',
        `Erreur lors du traitement: ${errorMessage.substring(0, 100)}. Veuillez réessayer.`,
      );
    }
  });
 ...
 */
async function updateShopFromReceipt(
  userId: string,
  receipt: ParsedReceipt,
): Promise<void> {
  try {
    const shopId =
      receipt.storeNameNormalized || normalizeStoreName(receipt.storeName || '');
    ...
    let receiptDate: admin.firestore.Timestamp;
    if (!receipt.date) {
    ...
    } else if ((receipt.date as any)?.toDate) {
      // Already a Timestamp
      receiptDate = receipt.date as admin.firestore.Timestamp;
    } else {
      // Assume Date object
      receiptDate = admin.firestore.Timestamp.fromDate(receipt.date as Date);
    }
    ...
    if (shopDoc.exists) {
      ...
      if (receipt.storeAddress && !existingData?.address) {
        updateData.address = receipt.storeAddress;
      }
      if (receipt.storePhone && !existingData?.phone) {
